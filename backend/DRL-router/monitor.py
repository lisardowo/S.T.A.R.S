...
class TrainingMonitoring:
    def plot_training_results(self, epochs, rewards, throughputs):
    ...
    def log_epoch_stats(self, file_path, data):
        """
    ...
    def visualize_satellite_routes(self, candidates, n_planes, n_sats, src, dst):
        """
...
class Monitor:
    ...
    def start_tx(self, env_now, algorithm, src, dst, size_bytes):
        ctx = {
            ...
            "hop_metrics": [],
            "success": False,
            "t_decision_start_ns": time.perf_counter_ns()
        }
    ...
    def record_hop(self, ctx, u, v, metrics):
        ...
        self.hist["throughput"].append((metrics or {}).get("link_throughput", 0.0))
    ...
    def end_tx(self, env_now, ctx, success, ref_cost=None, algorithm_cost=None):
        ...
        self.tx.append(ctx)
        self.counters[f"{ctx['algorithm']}_total"] += 1
        self.counters[f"{ctx['algorithm']}_success"] += int(success)
    ...
    def summary(self):
        ...
        for algorithm in algorithms:
            ...
            stretches = [t.get("path_stretch") for t in self.tx if t["algorithm"] == algorithm and t.get("path_stretch") is not None]
            succ = self.counters.get(f"{algorithm}_success", 0)
